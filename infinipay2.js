...
//  DataStrTODataObj
//  statitem_loaddetail
//  statitemprofile_clicked
//  statitemprofileitem_clicked
//  statitemprofileitem_form
//  define: strStatProfileItemFormID
//  define: strStatProfileFormID
//  load_mainnav
//  load_divbutton
//  init_divcontent
//  employee_statprofile
//  employee_statprofile_compare
//  slide_in
//  isdate
//  date_compare
...
const arryMainNavItem = [
    ...
    [],
    [],
    ['navitemprofileoption', 'Profile Option', 13, EmployeeProfileOption, undefined, dataobjlistdatarow_clicked, employeeprofileoption_loaddetail]
];
            ...
            setTimeout(function () {
                switch (dobjid) {
                    ...
                    case DataObjID.StatProfile:
                        if (obj_id) {
                            resolve(StatProfile.find(item => item.ID === obj_id || item.ID === parseInt(obj_id)));
                        } else {
                            resolve(StatProfile);
                        }
                        break;
                    case DataObjID.StatProfileEmployeeItem:
                        if (obj_id) {
                            resolve(StatProfileItem.find(item => item.ID === obj_id || item.ID === parseInt(obj_id)));
                        } else {
                            resolve(StatProfileItem);
                        }
                        break;
                    case DataObjID.EmployeeProfile:
                        let eprofile = EmployeeProfile.filter(item => {
                            return item.Employee === obj_id;
                        })
                        resolve(eprofile);
                        break;
                    // case DataObjID.EmployeeProfileOption:
                    //     resolve(EmployeeProfileOption);
                    //     break;
                    case DataObjID.EmployeeProfileItem:
                        resolve(EmployeeProfileItem);
                        break;
                    ...
                    default:
                        ...
                        if (obj_id) {
                            let dataobj = dobj.find(function (obj) {
                                return obj.ID === parseInt(this) || obj.ID === this;
                            }, obj_id);
                            resolve([dataobj]);
                        }
                        else {
...
async function employee_loaddetail(eeeid) {
    ...
    let divcontentemployee = init_divcontent(eeeid, dataobj_form, null);



    // load profile items //

    let arryeeeprofile = await dataobj_get(eeeid, DataObjID.EmployeeProfile);
    ...
    employeeprofileitemfull.forEach(profileitem => {
        ...
        profileitemX = arryeeeprofile.find(prfitem => prfitem.ProfileItem === profileitem.ID);
        if (profileitemX) {
        ...
        if (profileitem.Type === null || profileitem.Type === '' || isNaN(profileitem.Type)) {
            datacol2.push([datacolname, profileitem.Required, 'text']);
        }
        else {
            ...
            datacol2.push([datacolname, profileitem.Required, arrydataitemindex]);
            arrydataitemindex += 1;
        }
    })
    ...
    let arrybtn2 = [
        ['SUBMIT', 'Edit', dataobj_new, dataobj_url('put')],
        ['RESET', 'Reset', undefined, undefined],
    ];
    formparam.arrybutton = load_divbutton(false, arrybtn2);
    ...
    switch (operator) {
        ...
        case 4:
            if (valuedate) {
                ...
                if (parseFloat(value) >= parseFloat(criteria)) { console.log('true >='); return true; }
                else { console.log('false >='); return false; }
            }
            else {
                console.log('false >=');
        ...
        //  ()<>
        case 11:
            if (value.includes(criteria)) { console.log('false ()<>'); return false; }
...
function employee_statprofile(statprofile, eeeprofile) {
...
function DataStrTODataObj(strData) {
    //'1, Chinese; 2, Malay; 3, Indian; 4, Bangladesh'//
    let arrydataobj = [];
...
async function employeeprofileoption_loaddetail(currid) {
    ...
    dataobj_form[divcontent_formitemidx.FormDataObj] = dataobj;
    dataobj_form[divcontent_formitemidx.DataCol] = EmployeeProfileOptionDataCol;
    dataobj_form[divcontent_formitemidx.DataColObj] = null;

    let arrybtn = [
        [null, 'Edit', undefined, null],
        ['RESET', 'Reset', undefined, null],
        ['POST', 'New', undefined, null]
    ];
    dataobj_form[divcontent_formitemidx.Button] = load_divbutton(true, arrybtn);
...
async function statitem_loaddetail(statid) {
    ...
    statprofile_curr = sprofile.filter(item => {
        return item.StatItem === statid || item.StatItem === parseInt(statid);
    }, statid)
    ...
    let arrydatacol = StatProfileDataCol.map(datacol => {
    ...
    StatProfileDataCol.forEach(datacol => {
        emptydataobj[datacol[0]] = '';
    })
    objparam.arryjsondata = emptydataobj;
    ...
    objparam.arrybutton = load_divbutton(true, arrybtn2);
...
function load_divbutton(boolbtndir, arrysetbtn) {
    ...
    arrysetbtn.forEach(btn => {
        arrybtn = [];
        arrybtn[vssfnc_formpopparam_item.arrybutton.Type] = btn[0];
        arrybtn[vssfnc_formpopparam_item.arrybutton.Desc] = btn[1];
        arrybtn[vssfnc_formpopparam_item.arrybutton.OnClick] = btn[2];
        arrybtn[vssfnc_formpopparam_item.arrybutton.ActionURL] = btn[3];
        arrybtnparam.push(arrybtn);
    })
...
function init_divcontent(dataobj_ID, dataobj_form, dataobj_table) {
    ...
    if (!divcontentdetailX) {
        divcontentdetailX = document.createElement('div');
        divcontentdetailX.id = strDivContentDetailIDX;
        divcontentdetailX.classList.add(strClsContentDetail);
        divcontentdetailX.setAttribute('style', 'background-color:white;');
    }
    let divcontentdetailY = document.getElementById(strDivContentDetailIDY);
    if (!divcontentdetailY) {
        divcontentdetailY = document.createElement('div');
        divcontentdetailY.id = strDivContentDetailIDY;
        divcontentdetailY.classList.add(strClsContentDetail);
        divcontentdetailY.setAttribute('style', 'background-color:lightgray;');
    }
...
function slide_in(divin, divout, direction) {
    ...
    // handle outgoing //
    divout.classList.remove('slideinleft', 'slideoutleft', 'slideinright', 'slideoutright');
    ...
    // handle incoming //
    divin.classList.remove('slideinleft', 'slideoutleft', 'slideinright', 'slideoutright');
...
function fulllist_clicked() {
    ...
    let statprofile = StatProfile.filter(sttprf => { return sttprf.StatItem === 101 });
    let eeeprofile = EmployeeProfile.filter(epf => { return epf.Employee === '101' });

    console.log(statprofile);
    console.log(eeeprofile);
    let sttprofile = employee_statprofile(statprofile, eeeprofile);
...
async function statitemprofileitem_clicked(e) {
    ...
    let dataobj = await dataobj_get(this.dataset.id, DataObjID.StatProfileEmployeeItem);
...
async function statitemprofile_clicked(e) {
    ...
    if (profileeeeitem) {
        ...
        let dataobj = await dataobj_get(this.dataset.id, DataObjID.StatProfile);
        ...
        // get a copy of the array datacol //
        let arrydatacol = StatProfileItemDataCol.map(datacol => {
            return datacol.slice(0);
        })
        ...
        // let emptydataobj = new Object();
        // StatProfileItemDataCol.forEach(datacol => {
        //     emptydataobj[datacol[0]] = '';
        // })
        // objparam.arryjsondata = emptydataobj;
        ...
        //     ['POST', 'New', undefined],
        // ]
        // objparam.arrybutton = load_divbutton(true, arrybtn);
